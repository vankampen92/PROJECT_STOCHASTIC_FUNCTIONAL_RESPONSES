			                         ...
			                         Parameter_Model * P )
{
  ...
  /* This optimization can be implemented for any model. So far, 
     ...
     . MODEL=DIFFUSION_ECO_1B1P_SELF    TYPE_of_MODEL = 23
  ...
  */
  if (P->TYPE_of_MODEL == 21 || P->TYPE_of_MODEL == 17 || P->TYPE_of_MODEL == 18 || 
      ...
      P->TYPE_of_MODEL == 23) {
      /* This function initializes the Event_Delta_Matrix and the Event_Adjacence_List
         for each patch. The idea is to calculate these matrices only once, at the 
         beginning of the simulation, and then update them according to the flow of
         events that occur in the system. */
      /* This function should be used in conjunction with:
         Event_Delta_Matrix_Update();
         Temporal_Dynamics_Update();
         which are all functions intended to optimize the algorithm.  
      */
