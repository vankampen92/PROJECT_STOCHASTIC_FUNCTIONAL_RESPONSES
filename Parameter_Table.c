...
void P_A_R_A_M_E_T_E_R___T_A_B_L_E___A_L_L_O_C( Parameter_Table * Table )
{
  ...
  */
  ...
  /* Notice that No_of_RESOURCE_MAXIMUM is just a generic/historic name that gives the total number 
     of local variables 
  */
  Table->Solution_Stability_Type = (double *)calloc( MAXIMUM_No_of_MODEL_SOLUTIONS, sizeof(double) );
  Table->Vector_Model_Variables_Solution = (double **)calloc(MAXIMUM_No_of_MODEL_SOLUTIONS, sizeof(double *) ); 
  for(i = 0; i < MAXIMUM_No_of_MODEL_SOLUTIONS; i++) 
    Table->Vector_Model_Variables_Solution[i] = (double *)calloc( No_of_RESOURCES_MAXIMUM,  sizeof(double) );
  ...
  */
  ...
  #if defined(DIFFUSION_ECO_PLASMIDS) || defined(DIFFUSION_ECO_1B1P) || defined(DIFFUSION_ECO_1B1P_SELF)
...
void P_A_R_A_M_E_T_E_R___T_A_B_L_E___F_R_E_E( Parameter_Table * Table )
{
  ...
  */
  ...
  free(Table->Solution_Stability_Type);
  for(i = 0; i < MAXIMUM_No_of_MODEL_SOLUTIONS; i++) 
    free(Table->Vector_Model_Variables_Solution[i]);
  free(Table->Vector_Model_Variables_Solution);
...
void P_A_R_A_M_E_T_E_R___T_A_B_L_E___U_P_L_O_A_D( Parameter_Table * Table, int * Index_Output_Variables )
{
  ...
  Parameter_Values_into_Parameter_Table(Table);
  ...
  #if defined(DIFFUSION_ECO_PLASMIDS) || defined(DIFFUSION_ECO_1B1P) || defined(DIFFUSION_ECO_1B1P_SELF)
    assert(Table->TYPE_of_MODEL == 21 || Table->TYPE_of_MODEL == 22 || Table->TYPE_of_MODEL == 23); 
    ...
    */
    ...
    #endif
    #if defined DIFFUSION_ECO_1B1P_SELF
      assert(Table->TYPE_of_MODEL == 23); 
  ...
  */
    ...
    Resetting_Lambda_Delta_Vectors (Table);
  #endif 
  #ifndef DIFFUSION_ECO_1B1P_SELF
    Resetting_Lambda_Delta_Vectors (Table);
  #endif 
  /* END -------------------------------------------------*/
}
...
void Setting_Plasmid_Characteristic_Parameters (Parameter_Table * Table)
{
    ...
    assert(Table->TYPE_of_MODEL == 21 || Table->TYPE_of_MODEL == 22 || Table->TYPE_of_MODEL == 23);  /* 21: MODEL=DIFFUSION_ECO_PLASMIDS */
...
void Setting_Strain_Characteristic_Parameters (Parameter_Table * Table)
{
    ...
    if(Table->TYPE_of_MODEL == 21) {  /* 21: MODEL=DIFFUSION_ECO_PLASMIDS */
    ...
    else if (Table->TYPE_of_MODEL == 23 ){             /* 23: MODEL=DIFFUSION_ECO_1B1P_SELF */
      /* Plasmid Free Strain */
      Table->Beta_AP[0]  = Table->Beta_R;                                               /* Bacteria Cell Division Rate */
      Table->Eta_RP[0]   = Table->Lambda_R_1;                                           /* Bacteria Conjugation Rate   */
      Table->Delta_AP[0] = Table->Delta_R_0 + Table->Delta_R_1;                         /* Bacteria Death Rate         */
      ...
      */
      ...
      printf(" or MODEL=DIFFUSION_ECO_1B1P_SELF (TYPE_of_MODEL == 23)\n");
