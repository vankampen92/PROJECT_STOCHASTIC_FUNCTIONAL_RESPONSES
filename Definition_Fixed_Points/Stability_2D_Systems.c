...
double Calculate_Type_of_Stability_of_Solution( Parameter_Table * Table, 
                                                double * Vector_Stationarity )
{
  ...
  *  . Table, a pointer to the main data structure controling all model 
  *    and execution parameters
  *  . Vector_Stationarity, a pointer to the vector of model variables at the
  *    stationarity point. It is a double vector of size MODEL_STATE_VARIABLES.
...
double Evaluate_Turing_Condition( Parameter_Table * Table)
{
  ...
  */
  ...
  /* Check that the system is 2D */ 
  assert(Table->MODEL_STATE_VARIABLES == 2);

  /* Check that the system is stable */
  if (Type_of_Stability == 0.0 || Type_of_Stability == 1.0) {
    /* The system is unstable */
    Turing_Structures = 0.0;  /* No Turing instabililties */
    return(Turing_Structures);
  }

  /* BEGIN: Turing Stability Analysis */ 
  Tra = Trace(Table, Y); 
  Det = Determinant(Table, Y);
